...
char tab_mot_cle[20][20] = {
    "program", "begin", "var", "integer", "chart", "end", "if", "while",
...
/* ==== Fonction pour afficher le nom d'un symbole ==== */
...
unilex analex() {
    ...
    while (1) {
        ...
        case 0:
            ...
            if (car == ' ' || car == '\t' || car == '\r' || car == '\n')
                continue;
            else if (isalpha(car)) { z=0; ch[z++]=car; etat=1; }
            else if (isdigit(car)) { z=0; ch[z++]=car; etat=3; }
            else if (car=='<') etat=5;
            else if (car=='>') etat=9;
            ...
            else if (car=='\'') { z=0; etat=50; } // chaîne
            else if (feof(fp)) { symbole.ul=fin; return symbole; }
            break;

        case 1: // identificateur ou mot-clé
            car=carsuivant();
            if (isalpha(car) || isdigit(car)) ch[z++]=car;
            else { reculer(1); ch[z]='\0'; symbole.ul=unilexid(); symbole.att=rangerid(symbole.ul,&c);
                   printf("TOKEN: %-10s (CODE: %d)\n", nomSymbole(symbole.ul), symbole.ul); return symbole; }
            break;

        case 3: // nombre
            car=carsuivant();
            if (isdigit(car)) ch[z++]=car;
            else { reculer(1); ch[z]='\0'; symbole.ul=nb; symbole.att=atoi(ch);
                   printf("TOKEN: %-10s (CODE: %d)\n", nomSymbole(symbole.ul), symbole.ul); return symbole; }
            break;
    ...
    return symbole;
        ...
        case 50: // chaîne de caractères
            car=carsuivant();
            if (car=='\'') { ch[z]='\0'; symbole.ul=strlit; printf("TOKEN: '%s' (%s)\n", ch, nomSymbole(symbole.ul)); return symbole; }
            else ch[z++]=car;
            break;
...
void P() {
    if (symbole.ul==program) {
        ...
        INST_COMP();
        accepter(pt);
        if (symbole.ul != fin) erreur(); // ← Ajout ici
...
void DCL() { D1(); }
void D1() { if (symbole.ul==var) { accepter(var); L_ID(); accepter(dp); TYPE(); accepter(pv); D1(); } }
void L_ID() { accepter(id); L1(); }
void L1() { if (symbole.ul==v) { accepter(v); accepter(id); L1(); } }
void TYPE() { if (symbole.ul==integer) accepter(integer); else if (symbole.ul==chart) accepter(chart); else erreur(); }
void INST_COMP() {
    if (symbole.ul==begin) {
        accepter(begin);
        INST();
...
void L_INST1() { if (symbole.ul==pv) { accepter(pv); I(); L_INST1(); } }

void I() {
    if (symbole.ul==id) { accepter(id); accepter(aff); EXP_SIMPLE(); }
    else if (symbole.ul==iff) { accepter(iff); EXP(); accepter(then); I(); accepter(elsee); I(); }
    else if (symbole.ul==whilee) { accepter(whilee); EXP(); accepter(do); I(); }
    else if (symbole.ul==read || symbole.ul==readln) { accepter(symbole.ul); accepter(po); ARG_LIST(); accepter(pf); }
    else if (symbole.ul==write || symbole.ul==writeln) { accepter(symbole.ul); accepter(po); ARG_LIST(); accepter(pf); }
    else erreur();
}
...
void ARG_LIST() {
    if (symbole.ul==id || symbole.ul==strlit) {
        ...
        while (symbole.ul==v) {
            accepter(v);
            if(symbole.ul==id||symbole.ul==strlit) accepter(symbole.ul);
            else erreur();
        }
    }
}
...
void EXP_SIMPLE1() { if(symbole.ul==opadd) { accepter(opadd); TERME(); EXP_SIMPLE1(); } }
void TERME() { FACTEUR(); TERME1(); }
void TERME1() { if(symbole.ul==opmul) { accepter(opmul); FACTEUR(); TERME1(); } }
void FACTEUR() { if(symbole.ul==id) accepter(id); else if(symbole.ul==nb) accepter(nb); else if(symbole.ul==po) { accepter(po); EXP_SIMPLE(); accepter(pf); } else erreur(); }
